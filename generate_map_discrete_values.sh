...
EXP_ESCALA='z = d3.scaleThreshold().domain([0, 500, 2000, 5000, 8000]).range(d3.schemeBlues[5]), d.features.forEach(f => f.properties.fill = z(f.properties.renda)), d'
...
ndjson-map "$EXP_PROPRIEDADE" < pb-ortho-censo.ndjson | geo2topo -n tracts=- | toposimplify -p 1 -f | topoquantize 1e5 | topo2geo tracts=- | ndjson-map -r d3 -r d3=d3-scale-chromatic "$EXP_ESCALA" | ndjson-split 'd.features' | geo2svg -n --stroke none -w 1000 -h 600 > pb-chroropleth.svg
